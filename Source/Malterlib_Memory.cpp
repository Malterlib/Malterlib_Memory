	...
	ch8 const *gc_IgnoreFunctions[] =
		{
			...
			"-[CSPowerLog flushToPowerLog_locked_and_unlock]",
			...
			"__getCGDisplayListGetHashSymbolLoc",
			"swift_task_enterThreadLocalContext",
			"dynamic_cast_existential",
			"___realizeSettingsExtension",
			"util::UntypedTable",
			"AGGraphInternAttributeType",
			"AGSubgraphCreate2",
			"AGTypeApplyFields2",
			"AGGraphGetValue",
			...
			// Process and framework initialization that runs once
			"_dispatch_once_callout",
			"_NSEventThread",
			"_BeginEventReceiptOnThreadWithFilter",
			"BSObjCClassCreate",
			"+[BSXPCServiceConnectionProxy",
			"CGSPropertyListCreateWithSerializedData",
			"-[NSApplication _sendFinishLaunchingNotification]",
			"-[NSApplication setWindowsNeedUpdate:]",
			"_CheckEventsInited",
			"CFRunLoopAddSource",
			"-[BSServiceConnection activate]",
			"-[BSXPCServiceConnection configure:]",
			"-[RBSConnection _handshake]",
			"-[RBSXPCMessage invokeOnConnection",
			"-[WMClientWindowManager _connection]",
			"CA::Context::send_deferrals",
			"CA::Transaction::run_commit_handlers",
			"+[_NSCGSTransaction currentTransaction]",
			"-[BSServiceDispatchQueue",
			"-[BSXPCServiceConnection _",
			...
			"__SLSInstallRemoteContextNotificationHandlerV2",
			"_xpc_connection_mach_event",
			"CA::Transaction::ensure_implicit",
			"__27-[RBSConnection _handshake]",
			"_PerfAddRunLoopObserver",
			"CreateRunLoopSourceForQueue",
			"_LSCopyFrontApplication",
			"_LSCopyFrontUIApplication",
			"pushHighLevelEvent",
			"CA::Transaction::create",
			"__CFRunLoopServiceMachPort",
			"-[NSApplication _postSignpostsForCurrentPhase]",
			"-[NSApplication _eventBailOutForAppLaunchWillBegin]",
			"+[NSApplication(NSDisplayCycleObservation)",
			"alm_app_did_present",
			"__alm_execute_when_main_queue_idle_with_condition_block_invoke",
			"CFRunLoopRemoveObserver",
			"create_singleton",
			"ColorSyncProfileCreateSingleton",
			"_CFRuntimeRegisterClass",
			"__CGTypeRegisterInternal",
			"CGTypeCreateInstance",
			"-[_CFXPreferences init]",
			"weak_register_no_lock",
			"si_module_cache_byname",
			"_cfmp_source_invalidated",
			"FrameworkConfiguration::create",
			"-[NSApplication init]",
