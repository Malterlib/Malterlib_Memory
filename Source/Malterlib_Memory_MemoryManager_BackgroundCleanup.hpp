...
namespace NMib::NMemory
{
	...
	template <typename t_CParams>
	int64 TCMemoryManagerNumaArenaBackgroundCleanup<t_CParams>::fs_GetTimestamp()
	{
		// Unscaled timestamps share an epoch before and after time subsystem initialization.
		auto Return = NTime::NPlatform::fg_Timer_CyclesUnscaled();
			...
			(
				[this](NThread::CThreadObjectNonTracked *_pThread) -> aint
				{
					...
					fp32 WaitTime;
					{
						...
						LifeTime = MSLifetime * NTime::CSystem_Time::fs_CyclesUnscaledFrequency() / 1000;
					}
					int64 LifeTimeDecommit;
					{
						int64 MSLifetime = t_CParams::mc_BackgroundCleanupLifetimeDecommit;
						LifeTimeDecommit = MSLifetime * NTime::CSystem_Time::fs_CyclesUnscaledFrequency() / 1000;
					}
					int64 NextCleanup = fs_GetTimestamp() + LifeTime;
					...
					{
						int64 Time = fs_GetTimestamp();

						if (Time >= NextCleanup)
						{
							int64 Now = fs_GetTimestamp();
	...
	void TCMemoryManagerNumaArenaBackgroundCleanup<t_CParams>::f_StopThread()
	{
		...
		if (mp_pThread)
		{
			// Shutdown must make progress even when normal-priority work saturates the CPUs. The OS
			// can refuse the raise, and shutdown then runs at the priority the thread already has
			if (!mp_pThread->f_TrySetPriority(EExecutionPriority_Normal))
				DMibDTraceSafe("Could not raise the priority of the memory cleanup thread for shutdown" DMibNewLine);
