...
#endif
			ch8 const *m_pName;
			DMibIntrusiveLink(CMemoryCategory, NIntrusive::TCAVLLinkAggregate<>, m_Link);
...
#	define DMibMemoryImplementCategory(d_Name) NMib::NMem::CMemoryCategory g_MemoryCategory_##d_Name = {DMibStringize(d_Name)}

#	define DMibMemoryDefineDynamicCategory(d_Name) NMib::NMem::fg_Mem_DefineDynamicCategory(d_Name);

#	define DMibMemoryCategory(d_Name) NMib::NMem::CMemoryCategoryScope MemoryCategoryScope(&g_MemoryCategory_##d_Name)

#	define DMibMemoryDynamicCategory(d_pCategory) NMib::NMem::CMemoryCategoryScope MemoryCategoryScope(d_pCategory)
