...
namespace NMib
{
	namespace NMem
	{
		...
		class TCDebugMemoryManager
		{
			...
			mint m_Sequence;
			NContainer::TCMapWithPool<uint8 *, CMemoryBlock, NMib::CSort_Default, NMib::NMem::CAllocator_VirtualNoTrackingNoCommit, (2*1024*1024) / (sizeof(CMemoryBlock) + sizeof(void *)), NMib::NMem::CPoolType_Growing> m_Blocks;

			NContainer::TCMapWithPool<mint, CMemoryBlock, NMib::CSort_Default, NMib::NMem::CAllocator_VirtualNoTrackingNoCommit, (2*1024*1024) / (sizeof(CMemoryBlock) + sizeof(mint)), NMib::NMem::CPoolType_Growing> m_FreeBlocks;
			mint m_nFreeBlocks;
