...
namespace NMib
{
	namespace NMem
	{
		...
		NIntrusive::TCAVLTreeAggregate<CMemoryCategory::CLinkTraits_m_Link, CMemoryCategory::CCompare> g_AllCategories = {DAggregateInit};
		NAggregate::TCAggregateSimple<TCPoolGrowing<CDynamicMemoryCategory, 128, NMib::NThread::CNoLock, CAllocator_VirtualNoTracking>> g_CategoriesPool = {DAggregateInit};
		...
		struct CPerformanceCounters
		{
			...
			NContainer::TCMap<NStr::CStrNonTracked, PPERF_COUNTERSET_INSTANCE, NMib::CSort_Default, NMib::NMem::CAllocator_NonTrackedHeap> m_PerfCounters;
			...
			CPerformanceCounters()
			{
				...
				NDataProcessing::CUniversallyUniqueIdentifier HashRootGUID("354e81d6-242b-4b1e-badd-37ac3aceadbb", NDataProcessing::EUniversallyUniqueIdentifierFormat_Bare);
				
				NDataProcessing::CUniversallyUniqueIdentifier ProdiverGUID(NDataProcessing::EUniversallyUniqueIdentifierGenerate_StringHash, HashRootGUID, Executable + "/Provider");
				NDataProcessing::CUniversallyUniqueIdentifier CounterSetGUID(NDataProcessing::EUniversallyUniqueIdentifierGenerate_StringHash, HashRootGUID, Executable + "/CounterSet");

				m_ProdiverGUID = reinterpret_cast<GUID &>(ProdiverGUID);
				m_CounterSetGUID = reinterpret_cast<GUID &>(CounterSetGUID);

				NStr::CStrNonTracked ExecutableHash = NStr::fg_Format<NStr::CStrNonTracked>("{nfh,sj8,sf0}", NDataProcessing::CHash_MD5::fs_DigestFromData(Executable.f_GetStr(), Executable.f_GetLen()).f_FoldToInt<uint32>());
      ...
      <provider applicationIdentity = "{Executable}"
        ...
        <counterSet    guid        = "{CounterSetGUID}"
          ...
          <counter id           = "2"
				...
				Manifest = Manifest.f_Replace("{ProviderGUID}", ProdiverGUID.f_GetAsString(NDataProcessing::EUniversallyUniqueIdentifierFormat_Registry));
				Manifest = Manifest.f_Replace("{CounterSetGUID}", CounterSetGUID.f_GetAsString(NDataProcessing::EUniversallyUniqueIdentifierFormat_Registry));
		...
		NAggregate::TCAggregateSimple<CPerformanceCounters> g_Perfcounters = {DAggregateInit};
#endif
