...
namespace
{
	...
	struct CMemoryManagerZoneSmall
	{
		...
		malloc_zone_t_know_version m_MallocZone;
...
malloc_zone_t_know_version g_OriginalMallocs;
malloc_zone_t_know_version *g_pDefaultZone = nullptr;
...
malloc_zone_t_know_version g_MalterlibMallocZone =
	{
		.reserved1 = nullptr
		, .reserved2 = nullptr
...
void fg_MalterlibMallocOverrideEnable()
{
...
#endif
	{
		if (CSystem::ms_PlatformVersion >= 10'12'00)
		{
			...
			malloc_zone_t_know_version *pDefaultZone = (malloc_zone_t_know_version *)malloc_default_zone();
...
#			endif
		...
		auto fCreateZone = [&]<typename tf_CZone>()
			{
				...
				pMemoryManager->m_MallocZone = malloc_zone_t_know_version
					{
						.reserved1 = nullptr
						, .reserved2 = nullptr
	...
	/* memory allocation with an extensible binary flags option. Present in
	 * version >= 15 */
	assure_used DMibMalterlibOverrideMallocExport void *fg_Malterlib_malloc_zone_malloc_with_options_np(malloc_zone_t_know_version *_pZone, size_t _Align, size_t _Size, uint64_t _Options)
	{
#ifdef DMemoryManagerIsSame
		if (g_bForeignZone) [[unlikely]]
		{
